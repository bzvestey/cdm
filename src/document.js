import { addCompendiumItems, deleteCompendiumItems, getCompendiumModule, getDocsForCompendiumType, getPackForDocType, shouldDeleteMissingItems } from "./compendium.js";
import { FLAGS, MODULE_ID } from "./constants.js";

const ID_PREFIX_SETTING = "id_prefix"
...
export async function createDocuments(fileData) {
  ...
  if (byCompendium['Item']?.length) {
    ...
    if (deleteMissing) {
      itemsToDelete.push(...Object.values(cim).map(cim.data._id));
...
function updateDocData(doc) {
  ...
  if (doc.system && !(doc.system === game.system.id || (Array.isArray(doc.system) && doc.system.includes(game.system.id)))) {
  ...
  if (!dType || (doc.type === doc.docType && dType.includes("base")) || !dType.includes(doc.type)) {
...
function generateNewId() {
  const prefix = /*game.settings.get(MODULE_ID, ID_PREFIX_SETTING) ??*/ getCompendiumModule()
